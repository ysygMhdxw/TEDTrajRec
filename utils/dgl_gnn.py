...
from dgl.nn.pytorch import GATConv, GINConv, EGATConv, GraphConv, SAGEConv
...
class UnsupervisedGATv2(nn.Module):
    def __init__(
            self, node_input_dim, node_hidden_dim, edge_input_dim, num_layers, num_heads=8,
            last_activate=False):
        super(UnsupervisedGATv2, self).__init__()
        self.hid_dim = node_hidden_dim
        assert node_hidden_dim % num_heads == 0
        self.layers = nn.ModuleList(
            [
                GATv2Conv(
                    node_input_dim=node_input_dim if i == 0 else node_hidden_dim,
                    node_hidden_dim=node_hidden_dim // num_heads,
...
class UnsupervisedGraphSage(nn.Module):
    def __init__(
            self, node_input_dim, node_hidden_dim, edge_input_dim, num_layers,
            last_activate=False):
...
class GATv2Conv(nn.Module):
    ...
    Parameters
    ----------
    node_input_dim : int, or pair of ints
    ...
    def __init__(self,
        ...
        self._in_src_feats, self._in_dst_feats = expand_as_pair(node_input_dim)
        self._out_feats = node_hidden_dim
        self._allow_zero_in_degree = allow_zero_in_degree
        if isinstance(node_input_dim, tuple):
            self.fc_src = nn.Linear(
                self._in_src_feats, node_hidden_dim * num_heads, bias=bias)
            self.fc_dst = nn.Linear(
                self._in_dst_feats, node_hidden_dim * num_heads, bias=bias)
        else:
            self.fc_src = nn.Linear(
                self._in_src_feats, node_hidden_dim * num_heads, bias=bias)
            if share_weights:
                ...
                self.fc_dst = nn.Linear(
                    self._in_src_feats, node_hidden_dim * num_heads, bias=bias)
        self.attn = nn.Parameter(torch.FloatTensor(size=(1, num_heads, node_hidden_dim)))
        self.feat_drop = nn.Dropout(feat_drop)
        self.attn_drop = nn.Dropout(attn_drop)
        self.leaky_relu = nn.LeakyReLU(negative_slope)
        if residual:
            if self._in_dst_feats != node_hidden_dim:
                self.res_fc = nn.Linear(
                    self._in_dst_feats, num_heads * node_hidden_dim, bias=bias)
            else:
